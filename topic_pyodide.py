"""Topic modeling for Pyodide using pre-computed embeddings.

This module implements a topic modeling pipeline that runs entirely in the browser
using Pyodide. Embeddings are generated client-side via JavaScript (Transformers.js),
then this module performs dimensionality reduction, clustering, and topic extraction.
"""
...
class PyodideTopicModeler:
    """A topic modeler for Pyodide using pre-computed embeddings.

    This class implements a topic modeling pipeline optimized for in-browser execution
    with client-side embedding generation (via Transformers.js). The pipeline receives
    pre-computed embeddings and performs dimensionality reduction, clustering, and
    topic word extraction using scikit-learn only.
    """
    ...
    def fit_transform(
        ...
        """Fit the topic model using pre-computed embeddings.

        This method performs dimensionality reduction via PCA, clustering via DBSCAN
        or KMeans, and topic word extraction via TF-IDF.

        Args:
            embeddings: List of embedding vectors from Transformers.js (shape: [n_docs, embedding_dim]).
            documents: List of text documents (used only for topic word extraction).
        ...
        Raises:
            ValueError: If fewer than 2 documents or mismatched dimensions.
        """
        if len(documents) < 2:
        ...
        self.embeddings_ = embeddings_array
    ...
    def export_model_state(self) -> str:
        ...
        The state includes cluster labels, topic words, embeddings, and documents.
        This JSON can be saved to localStorage or IndexedDB in the browser and
        later restored with import_model_state().

        Returns:
            JSON string containing cluster labels, topic words, embeddings, and documents.
    ...
    def import_model_state(self, json_str: str) -> None:
        ...
        required_fields = ["cluster_labels", "topic_words", "documents", "embeddings"]
        ...
        self.embeddings_ = np.array(state["embeddings"], dtype=np.float32)
        self._is_fitted = True
